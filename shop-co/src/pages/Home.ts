...
export function renderHomePage(appElement: HTMLElement): void {
  ...
  fashion.className = 'pb-0 bg-gray-100';
  ...
  brands.className = 'py-8 bg-black lg:py-12';
  brands.innerHTML = `
  ...
  categoriesSection.className = 'container px-4 py-16 mx-auto lg:px-24';

  const categoriesTitle = document.createElement('h2');
  categoriesTitle.className = 'mb-12 text-4xl font-bold text-center md:text-5xl font-poppins';
  categoriesTitle.textContent = 'Categories';
  categoriesSection.appendChild(categoriesTitle);

  const categoriesGrid = document.createElement('div');
  categoriesGrid.id = 'categories-grid';
  categoriesGrid.className = 'grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-8';
  ...
  categoriesList.forEach(name => {
    ...
    el.className = 'bg-gray-100 p-6 cursor-pointer flex items-center justify-center text-center min-h-[160px]';

    const title = document.createElement('h3');
    title.className = 'text-xl font-semibold text-gray-800 break-words md:text-2xl font-poppins';
  ...
  const shopNowBtn = document.getElementById('shop-now-button');
